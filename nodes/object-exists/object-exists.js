module.exports = function S3ObjectExists(RED) {
  const nodeInstance = instanceNode(RED);
  RED.nodes.registerType("Object Exists", nodeInstance);
};
...
function inputHandler(n, RED) {
  return async function nodeInputHandler(msg, send, done) {
    ...
    try {
      ...
      send(msgClone);
      this.status({ fill: "green", shape: "dot", text: "Object exists!" });
    } catch (err) {
      // If error occurs
      this.status({ fill: "yellow", shape: "dot", text: "Object does not exist!" });
      // Replace the payload with null
      msgClone.payload = false;
      // Append the object
      // key to the message object
      msgClone.key = payloadConfig.Key;
      send(msgClone);
